...
export function RewardCard({ reward, canRedeem, onRedeem, onEdit, onDelete, busy }: { reward: Reward; canRedeem: boolean; onRedeem?: () => void; onEdit?: () => void; onDelete?: () => void; busy?: boolean }) {
  const oneTimeRedeemed = reward.redeemMode !== "multiple" && reward.redeemed;
  return (
    <div className={cn("rounded-3xl bg-white p-4 shadow-soft dark:bg-slate-800", oneTimeRedeemed && "opacity-60")}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-xs font-extrabold uppercase text-slate-400 dark:text-slate-400">{reward.redeemMode === "multiple" ? "Redeem multiple times" : "Redeem once"}</p>
          </div>
        </div>
      ...
      <div className="mt-4 flex items-center gap-2">
        {onRedeem ? <button disabled={oneTimeRedeemed || !canRedeem || busy} onClick={onRedeem} className="min-h-12 flex-1 rounded-2xl bg-blueberry px-4 py-3 font-black text-white disabled:bg-slate-200 disabled:text-slate-500 dark:disabled:bg-slate-700">{oneTimeRedeemed ? "Redeemed" : busy ? "Processing" : "Redeem reward"}</button> : null}
