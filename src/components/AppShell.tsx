...
import { Gift, Home, Settings, Sparkles } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#f7fbff] text-slate-900 dark:bg-slate-900 dark:text-white">
      <header className="sticky top-0 z-30 border-b border-white/70 bg-[#f7fbff]/90 px-4 py-3 backdrop-blur dark:border-slate-700 dark:bg-slate-900/90">
        <div className="mx-auto flex max-w-5xl items-center justify-between">
          ...
          <div className="hidden items-center gap-2 rounded-full bg-white p-1 shadow-soft dark:bg-slate-800 sm:flex">
            {navItems.map((item) => {
      ...
      <nav className="safe-bottom fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white/95 px-4 py-2 backdrop-blur dark:border-slate-700 dark:bg-slate-900/95 sm:hidden">
